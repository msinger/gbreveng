...
[dnl
# Rednex Game Boy Development System checksum fixer
AC_ARG_VAR(FIX, [GameBoy checksum fixer (overrides auto detection)])
if test -z "$FIX"; then
	AC_PATH_PROG(FIX, rgbfix, [])
	if test -z "$FIX"; then
		AC_MSG_ERROR([rgbfix not found])
	fi
fi
AC_ARG_VAR(FIXFLAGS, [Additional arguments passed to GameBoy checksum fixer])
])dnl
dnl
